...
motusUpdateTagDB = function(sql, countOnly=FALSE) {
    ...
    if (! countOnly) {
        ...
        ## get records for all new batches

        repeat {
            b = srvBatchesForTagProject(projectID=projectID, ts=ts)
            if (nrow(b) == 0)
                break
            ...
            for (bi in 1:nrow(b)) {
                ...
                ## get all runs which might be created or modified by this batch
                ...
                ## get hits for this batch, starting after the largest hitID we already have
                ...
                ## get GPS fixes for this batch, starting after the largest TS for which we
                ## already have a fix
                ...
                ## mark the transfers for this batch as complete by writing a record to the batches table
                dbWriteTable(sql$con, "batches", b[bi,], append=TRUE, row.names=FALSE)
            }
            ts = max(b$ts)
        }

        ## get metadata for tags and their deployments
        ...
        ## get metadata for tag ambiguities
