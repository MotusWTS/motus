...
#'
#' # This creates an active binding for the symbol "userLogin" in the
#' environment "motus_vars". 
#' # If the variable `motus_vars$userLogin`is requested in code, the user will 
#' # be prompted to enter a value if there is no current value.
#' 
#' @noRd

sessionVariable <- function(name, info = name, env = motus_vars, 
                            class = "character", val = NULL) {
  getSet <- function(val) {
    if ( ! missing(val)) {
