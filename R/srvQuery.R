...
#'   which will be included automatically from `motus_vars$authToken` Otherwise,
#'   an authentication token will be requested using the credentials in
#'   `motus_vars[c("userLogin", "userPassword")]`
#' @param url Character. API url, defaults to `motus_vars$dataServerURL`
#'
#' @return the result of sending the request to the data server API.  The result
#'   is a JSON-format character scalar if `json` is `TRUE`; otherwise it is an R
#'   list with named components, extracted from the JSON return value.
#'
