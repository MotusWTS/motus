...
```{r}
alltags_filtered <- anti_join(tbl(sql.motus, "alltags"), bad_runs, by = "runID")
```
...
```{r}
alltags_filtered2 <- filterByActivity(sql.motus, return = "all") %>% collect()
```
...
```{r}
alltags_filtered3 <- filterByActivity(sql.motus, return = "all", view = "alltagsGPS")
```
...
```{r}
alltags_filtered4 <- filterByActivity(sql.motus, return = "all") %>%
...
```{r}
relaxed <- filterByActivity(sql.motus, minLen = 2, maxLen = 4, 
...
```{r}
strict <- filterByActivity(sql.motus, minLen = 4, maxLen = 10, 
...
max_runlen <- tbl(sql.motus, "alltags") %>%
  collect() %>%
