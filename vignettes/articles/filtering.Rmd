...
[Chapter 5 - Data cleaning](05-data-cleaning.html) is a good place to start for a walk-through), and in this article we'll cover in more detail two methods of filtering out dubious hits based on radio noise (activity) and run lengths. 
...
In general, short runs (with a length of only 2 or 3) have a relatively high probability of being false positive detections.
In contrast, long runs (with a length of 5 or more) have a high probability of being true positives. 
Therefore, runs with a length 3 or less are conservatively considered invalid, and runs with a length of 5 or more are considered valid.
...
Noisy sites are categorized as those with many runs (>= 100) and a high ratio of runs with lengths of 2 at a given time (>= 85%). 
...
1. Easiest is the field/column called `motusFilter` found in the `runs` table [^1]
...
```{r}
bad_runs <- tbl(tags, "runs") %>%
  filter(motusFilter == 0)
```

Now use `anti_join()` to remove those runs from the `alltags` view
```{r}
alltags_filtered <- anti_join(tbl(tags, "alltags"), bad_runs, by = "runID")
```
...
However, users can fine-tune the filter by adjusting all these cutoffs.
...
```{r}
alltags_filtered2 <- filterByActivity(tags, return = "good")
```
...
```{r}
alltags_filterd3 <- filterByActivity(tags, return = "all", view = "alltagsGPS")
```
...
We'll convert `ts` to a datetime format with `as_datetime()` from the lubridate package and we'll use the `month()` function to pull out months.
...
[^2]: 
Future version of the `motus` package should give users access to specific information on why each run was filtered (activity/noise or a manual assessment).
