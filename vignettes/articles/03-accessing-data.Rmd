...
This will also create an SQL object in your R environment called `sql_motus`

```{r, eval = FALSE}
sql_motus <- tagme(projRecv = 176, new = TRUE, update = TRUE)
```
...
```{r, eval = FALSE}
sql_motus <- tagme(projRecv = 176, new = TRUE, update = TRUE, dir = "./data/")
```
...
sql_motus <- tagme(projRecv = proj.num, new = TRUE, update = TRUE)
```
...
Now that we've downloaded our data as an SQLite database and loaded it into an R object called `sql_motus`, we want to access the tables stored within. 
...
For example, retrieve the virtual `alltags` table from our `sql_motus` SQLite file.
```{r}
...
```{r}
sql_motus <- tagme(projRecv = 176, new = FALSE, update = TRUE, dir = "./data/") 
```
...
sql_motus <- tagme(projRecv = 176, new = FALSE, update = FALSE, dir = "./data")
```
...
sql_motus <- tagme(176, update = TRUE, dir = "./data")  
