...
gps_tags <- dbplyr::src_dbi(con = gps_tags)
...
For example, let's work with a subset of the alltags view, including only Red Knots.

```{r}
knots <- tbl(gps_tags, "alltags") %>%
  filter(speciesEN == "Red Knot")
```

Now let's retrieve the daily median location of GPS points for these data.
Note that we use both the original database `gps_tags` as well as the data subset `knots`. 

```{r}
index_GPS <- getGPS(src = gps_tags, data = knots)
```
...
Note that at this point, we need to use `collect()` to ensure that `knots` is 'flat' (i.e. a data frame, not a database; see [Converting to flat data](03-accessing-data.html#converting-to-flat-data) for more details).

```{r}
knots_GPS <- left_join(collect(knots), index_GPS, by = "hitID")
```
...
```{r}
index_GPS <- getGPS(src = gps_tags, data = knots, by = 60)
index_GPS
```    
...
```{r}
index_GPS <- getGPS(src = gps_tags, data = knots, by = "daily")
index_GPS
```
...
```{r}
index_GPS <- getGPS(src = gps_tags, data = knots, by = "closest", cutoff = 120)
index_GPS
```
...
```{r}
index_GPS <- getGPS(src = gps_tags, data = knots, keepAll = TRUE)
```
