...
**Note:** if you have GPS points in your data, you may wish to check out the article on [Working with GPS points](gps.html), 
particularly [the section](gps.html#using-gps-locations) which details how you might wish to combine create new lat/lons with a combination of `recvDeployLat`/`recvDeployLon` and `gpsLat`/`gpsLon'`
...
To do so, we collect the motusTagID that we want to remove.
```{r}
...
And plot the ambiguous detections by date and receiver. 
...
However, if your tags have GPS, you may wish to use a more precise lat/lon. 
