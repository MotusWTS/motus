...
df.summary <- tbl.alltags.sub %>%
  ...
  group_by(motusTagID, runID, recvDeployName, ambigID, 
           tagDeployLon, tagDeployLat, recvDeployLat, recvDeployLon) %>%
...
tbl.alltags.sub %>%
  ...
  count(motusTagID, tagDeployStart, tagDeployEnd, 
           tagDeployLat, tagDeployLon, month)
```
...
ggplot(data = df.ambig.171, aes(x = time, y = sig, colour = as.factor(port))) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) + 
  geom_point() + 
...
ggplot(data = df.ambig.114, aes(x = time, y = sig, colour = LatLonStationName)) +
  geom_point() + 
  theme_bw() 
```
...
ggplot(data = df.ambig.106, aes(x = time, y = sig, colour = col)) +
  theme_bw() +
  geom_point() + 
...
ggplot(data = df.ambig.56, aes(x = time, y = recvDeployLon, colour = col)) +
  theme_bw() +
  geom_point() + 
  scale_colour_discrete(name="Lat/Lon and\nStation Name") 
```
...
ggplot(data = filter(df.ambig.56, time > time.begin, time < time.end),
...
ggplot(data = df.56.tmp, aes(x = time, y = sig, colour = col)) + 
  theme_bw() +
  geom_point() + 
  scale_colour_discrete(name = "Lat/Lon and\nStation Name") + 
  facet_wrap(~ date, scales = "free_x")
